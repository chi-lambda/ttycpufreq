...
namespace ttycpufreq
{
    class Program
    {
        ...
        {
            Console.WriteLine("{0}   {1:N2}, {2:N2}, {3:N2}   {4:HH:mm:ss}       ttycpufreq, v{5}\n",
                hostname,
                readings.Select(r => r.MinFreq).Min(),
                readings.Select(r => r.AvgFreq).Average(),
                readings.Select(r => r.MaxFreq).Max(),
        ...
        private static void ShowFreqs(int maxHeight)
        {
            int maxFreq = readings.Select(reading => reading.MaxFreq).Max();

            foreach (var reading in readings)
            {
                reading.ComputeHeights(maxFreq, maxHeight);
            }

            for (int height = 0; height <= maxHeight; height++)
            {
                Console.Write("{0,6:F2}   ", maxFreq * (maxHeight - height) / maxHeight);
                var readingNode = readings.First;
                while (readingNode != null)
                {
                    var color = ConsoleColor.Black;
                    char ch = ' ';
                    ch = GraphCharacter(readingNode, height, n => n.Value.MaxHeight);
                    if (ch != ' ')
                    {
